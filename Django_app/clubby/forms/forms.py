...
class SignupForm(UserCreationForm):
    ...
    birth_date = forms.DateField(widget=DateInput(attrs={'class': 'datepicker','readonly':'readonly','style': 'color: white'}), initial= (datetime.datetime.now()-datetime.timedelta(days=365*18)).date())
...
class EventModelForm(ModelForm):
    name = forms.CharField(max_length=50, required=True, help_text=_('Required. 50 character max' ))
    start_date = forms.DateField(widget=DateInput(attrs={'class': 'datepicker','readonly':'readonly','style': 'color: white'}), initial= datetime.date.today)
