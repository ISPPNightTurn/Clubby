...
class Club(models.Model):
    ...
    name = models.CharField(
        max_length=50, help_text='Enter the name of your club.')
    address = models.CharField(
        max_length=200, help_text='Enter the full address so google maps can find it.')
    max_capacity = models.IntegerField(
        help_text='The capacity of your club, you\'re responsible for the enforcement of this number.')
    NIF = models.CharField(
        max_length=10, help_text='Company number for the club')
...
class Ticket(models.Model):
    price = models.DecimalField(
        decimal_places=2, max_digits=5)  # 999,99 es el maximo
    category = models.CharField(
        max_length=40, help_text='The name of the type of ticket you are trying to sell.', default='Basic')
    description = models.TextField(
        help_text='Decribe what this ticket entices.', default="this allows you to enter the party.")
...
class CreateTicket(models.Model):
    price = models.DecimalField(
        decimal_places=2, max_digits=5, default=1)  # 999,99 es el maximo
    category = models.CharField(
        max_length=40, help_text='The name of the type of ticket you are trying to sell.', default='Basic')
    description = models.TextField(
        help_text='Decribe what this ticket entices.', default="this allows you to enter the party.")
    size = IntegerRangeField(min_value=1, max_value=50,
                             default=1, help_text='Number of tickets. (Max)')
    event = models.ForeignKey(Event, on_delete=models.CASCADE)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, null=True, blank=True)
...
class Rating(models.Model):
    ...
    recommended = models.BooleanField(
        help_text='would you recommend this club?')
    club = models.ForeignKey(Club, on_delete=models.CASCADE)
